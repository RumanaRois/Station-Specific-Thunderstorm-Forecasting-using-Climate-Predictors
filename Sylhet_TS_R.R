...
ggplot(D1, aes(x = Date, y = MT)) +
  ...
  labs(
    x = "Year",
    y = "Monthly Total TS Days in Sylhet"
  ) +
...
time_series_plot = ggplot(data_long, aes(x = Date, y = Value, color = Component)) +
  ...
  labs(
    x = "Year",
    y = "Monthly Total TS Frequency in Sylhet",
    color = "Components",
  ) +
  theme_minimal(base_size = 12) +
  theme(
...
B1 = ggplot(D1, aes(x = M, y = MT, fill = M)) +
...
B1 + scale_fill_viridis_d(option = "plasma") # Options: "magma", "inferno", "plasma", "viridis"


library(patchwork)
#tiff("Figure 1.tiff", width = 9, height = 4, units = "in", res = 300)
time_series_plot + B1
#dev.off()
...
# Save to Excel
write.xlsx(errors, "Sylhet_Errors.xlsx")
